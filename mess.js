import baileys, { jidNormalizedUser, extractMessageContent, areJidsSameUser, downloadMediaMessage, generateForwardMessageContent, generateWAMessageFromContent } from "villainwa"
...
export default async function serialize(guc, msg, store) {
   const m = {}
   ...
   if (msg.key) {
      ...
      m.sender = jidNormalizedUser(m.fromMe ? guc.user.id : m.isGroup ? m.participant : m.from)
   }

   if (m.isGroup) {
      m.metadata = store.groupMetadata[m.from] || await guc.groupMetadata(m.from)
      ...
      m.isBotAdmin = m.isGroup && !!m.groupAdmins.find((member) => member.id === jidNormalizedUser(guc.user.id))
   }
      ...
      if (m.msg?.contextInfo?.quotedMessage) {
         ...
         if (m.quoted.message) {
            ...
            m.quoted.key = {
               ...
               fromMe: areJidsSameUser(jidNormalizedUser(m.msg?.contextInfo?.participant), jidNormalizedUser(guc?.user?.id)),
   ...
   m.reply = async (text, options = {}) => {
      if (typeof text === "string") {
         return await guc.sendMessage(m.from, { text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options })
